...
namespace LayerDao
{
    public class ProjectDao
    {
        ...
        {
            var query = $"select Project.*,MobileSpaces.Name as MobileSpaceName,Customer.Name as CustomerName ,Mobile.Name as MobileName from Project Left Outer join MobileSpaces on Project.MobileSpaceId = MobileSpaces.id Left Outer join Customer on Customer.Id = Project.CustomerId Left Outer join Mobile on MobileSpaces.MobileId = Mobile.Id;";
            return QueryExecutor.List<Project>(query);
        }
        public static Project GetById(long id)
        {
            var query = $"select Project.*,MobileSpaces.Name as MobileSpaceName,Customer.Name as CustomerName ,Mobile.Name as MobileName from Project Left Outer join MobileSpaces on Project.MobileSpaceId = MobileSpaces.id Left Outer join Customer on Customer.Id = Project.CustomerId Left Outer join Mobile on MobileSpaces.MobileId = Mobile.Id where Project.Id = {id}; ";
