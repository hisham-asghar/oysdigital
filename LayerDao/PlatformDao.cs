...
namespace LayerDao
{
    public class PlatformDao
    {
        ...
        {
            string sql = $"Insert Into dbo.Platforms (Name,IconeUrl,IsActive,CreatedBy,ModifiedBy,OnCreated,OnModified)" +
                $" output INSERTED.PlatforfId as Result VALUES " +
        ...
        {
            string sql = $"UPDATE dbo.Platforms Set Name='{p.Name}',IconUrl='{p.IconUrl}',IsActive='{p.IsActive}',ModifiedBy='{p.ModifiedBy}',OnModified='{p.OnModified}' output INSERTED.PlatforfId as Result where (PlatformId={p.PlatformId})";

            return QueryExecutor.FirstOrDefault<TemplateClass<long>>(sql).Result == 0 ? false : true; ;
        }
        public static bool Delete(long Id)
        {
            string sql = $"DELETE FROM dbo.Platforms output deleted.PlatforfId as Result WHERE PlatformId = {Id};";
            return QueryExecutor.FirstOrDefault<TemplateClass<long>>(sql).Result == 0 ? false : true; ;
        }
    }
}
